...
T.create_menu = function(title, items, handler)
  if not nui_ok then
    return
  end
...
T.notify = function(msg, level, opts)
  opts = opts or {}
  level = vim.log.levels[level:upper()]
  if type(msg) == "table" then
    msg = table.concat(msg, "\n")
  end
  local nopts = { title = "Themes" }
  if opts.once then
...
T.load = function(hl_groups)
  ...
  local bufferline_icon_group = require("themes.theme.plugins.bufferline").setup_bufferline_icon()
  draw(bufferline_icon_group)
  -- draw bufferline icons
  vim.api.nvim_create_autocmd({ "BufEnter", "BufReadPost", "BufWinEnter", "BufRead" }, {
    pattern = "*",
    callback = function()
      if vim.g.colors_name ~= "themes" then
        return true
      end
      bufferline_icon_group = require("themes.theme.plugins.bufferline").setup_bufferline_icon()
      draw(bufferline_icon_group)
    end,
  })
end
